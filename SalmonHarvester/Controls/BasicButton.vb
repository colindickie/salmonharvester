...
#Region "Events"
            ...
            Try
                Select Case _OutputType
                    Case MfgControl.AdvancedHMI.Controls.OutputType.MomentarySet
                        _CommComponent.Write(_PLCAddressClick, 1)
                    ...
                    Case MfgControl.AdvancedHMI.Controls.OutputType.MomentaryReset
                        _CommComponent.Write(_PLCAddressClick, 0)
                        If m_MinimumHoldTime > 0 Then MinHoldTimer.Enabled = True
                        If m_MaximumHoldTime > 0 Then MaxHoldTimer.Enabled = True
                    Case MfgControl.AdvancedHMI.Controls.OutputType.SetTrue : _CommComponent.Write(_PLCAddressClick, 1)
                    Case MfgControl.AdvancedHMI.Controls.OutputType.SetFalse : _CommComponent.Write(_PLCAddressClick, 0)
                    Case MfgControl.AdvancedHMI.Controls.OutputType.Toggle
                        Dim CurrentValue As Boolean
                        CurrentValue = _CommComponent.Read(_PLCAddressClick, 1)(0)
                    ...
                    Case MfgControl.AdvancedHMI.Controls.OutputType.WriteValue
                        _CommComponent.Write(_PLCAddressClick, m_ValueToWrite)
                End Select
    ...
    Private Sub ReleaseValue()
        Try
            Select Case _OutputType
                Case MfgControl.AdvancedHMI.Controls.OutputType.MomentarySet : _CommComponent.Write(_PLCAddressClick, 0)
                Case MfgControl.AdvancedHMI.Controls.OutputType.MomentaryReset : _CommComponent.Write(_PLCAddressClick, 1)
            End Select
        Catch ex As MfgControl.AdvancedHMI.Drivers.Common.PLCDriverException
            If ex.ErrorCode = 1808 Then
                DisplayError("""" & _PLCAddressClick & """ PLC Address not found")
...
#Region "Subscribing and PLC data receiving"
    ...
    Private Sub SubscribeToCommDriver()
        If Not DesignMode And IsHandleCreated Then
            ...
            If _PLCAddressVisible <> "" Then
                Dim PLCAddress As String = _PLCAddressVisible
                If PLCAddress.ToUpper.IndexOf("NOT ") = 0 Then
                    PLCAddress = _PLCAddressVisible.Substring(4).Trim
                    InvertVisible = True
                Else
                    InvertVisible = False
                End If
    ...
    Private Sub PolledDataReturnedHighlight(ByVal sender As Object, ByVal e As MfgControl.AdvancedHMI.Drivers.Common.PlcComEventArgs)
        Try
            If _OutputType = AdvancedHMI.Controls.OutputType.WriteValue Then
                Highlight = (e.Values(0) = m_ValueToWrite)
            Else
                Highlight = e.Values(0)
            End If
        Catch
    ...
    Private Sub PolledDataReturnedBackColorChange(ByVal sender As Object, ByVal e As MfgControl.AdvancedHMI.Drivers.Common.PlcComEventArgs)
        Try
            If e.Values(0) Then
    ...
    Private Sub PolledDataReturnedSelectTextAlternate(ByVal sender As Object, ByVal e As MfgControl.AdvancedHMI.Drivers.Common.PlcComEventArgs)
        Try
            SelectTextAlternate = e.Values(0)
        Catch
